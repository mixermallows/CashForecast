...
async function checkAuthState() {
  ...
  if (session) {
    ...
    document.querySelector('.nav-btn').classList.add('active');
    plannerMonth = currentMonth;
    document.getElementById('profile-name').innerText = session.user.user_metadata.full_name || "User";
...
async function deleteSelected() {
  ...
  await supabaseClient.from('transactions').delete().in('id', ids);
  toggleSelectMode(); fetchTransactions();
}
async function deleteSingleItem() {
  const id = document.getElementById('editing-id').value; if (!id) return;
...
function calcInstallment() { const p=parseNumber(document.getElementById('form-total-debt').value); const n=parseInt(document.getElementById('form-months').value)||0; const r=parseFloat(document.getElementById('form-interest').value)||0; if(p>0&&n>0){ let m=0; if(r===0) m=p/n; else { let i=(r/100)/12; m=p*i*Math.pow(1+i,n)/(Math.pow(1+i,n)-1); } document.getElementById('calc-monthly').innerText=Math.round(m).toLocaleString()+' ฿'; document.getElementById('form-amount').value=Math.round(m).toLocaleString(); document.getElementById('calc-result').classList.add('show'); } else document.getElementById('calc-result').classList.remove('show'); }

async function saveItem() {
  const id=document.getElementById('editing-id').value; const name=document.getElementById('form-name').value.trim(); const amount=parseNumber(document.getElementById('form-amount').value); const totalDebt=parseNumber(document.getElementById('form-total-debt').value);
  if(!name||amount<=0){alert('กรุณากรอกชื่อรายการและยอดเงินให้ถูกต้อง');return;}
  const payload = { month_key: plannerMonth, name, amount, type: currentSheetType, category: selectedCategory, occurrence: currentOccurrence, total_debt: totalDebt };
  try {
    const { data: { session } } = await supabaseClient.auth.getSession(); 
    payload.user_id = session?.user?.id || null; 
    if(id) { const { error } = await supabaseClient.from('transactions').update(payload).eq('id', id); if(error) throw error; } 
